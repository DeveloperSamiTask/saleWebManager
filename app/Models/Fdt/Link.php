...
class Link extends Model
{
    ...
    public $table = 'fdt_courtesy_link';
    protected $fillable = [
        'code',
        'food',
        'lastname',
    ...
    {
        return $this->hasMany(Combo::class, 'purchase_link_id');
    }

    public static function getList($startDate, $endDate, $isChecked)
    {
        $filterField = $isChecked == '1' ? 'date_issue' : 'created_at';
        ...
        foreach ($coupons as $row) {
            ...
            foreach ($row->combos as $combo) {
                ...
                $validatedCombos += ComboValidation::where('purchase_link_combo_id', $combo->id)
                    ->sum('validated_qty');
            }
